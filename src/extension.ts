...
export function activate(context: vscode.ExtensionContext) {
  ...
  const translateCommand = vscode.commands.registerCommand(
    "xixifuTranslate.translateSelection",
    async () => {
      await translateSelectedText();
    }
  );

  const showNamingCommand = vscode.commands.registerCommand(
    "xixifuTranslate.showNamingOptions",
    async () => {
  ...
  const copyCommand = vscode.commands.registerCommand(
    "xixifuTranslate.copyToClipboard",
    async (text: string) => {
  ...
  const clearStatusBarCommand = vscode.commands.registerCommand(
    "xixifuTranslate.clearStatusBar",
    () => {
        ...
        try {
          ...
          if (
            ...
            if (selectedText !== lastSelectionText) {
              ...
              console.log(
                `[Selection] Processing text: "${selectedText}" (length: ${selectedText.length})`
              );
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      if (trimmedText.length === 0) {
        selectionDebugger.logTextRetrievalAttempt(
          attempt + 1,
          selection,
          text,
          false,
      ...
      if (text.length !== expectedLength) {
        ...
        console.log(
          `[Selection] Attempt ${attempt + 1}: Length mismatch - got ${
            text.length
      ...
      console.log(
        `[Selection] Attempt ${
          attempt + 1
        }: Successfully got text "${trimmedText}" (${trimmedText.length} chars)`
      );
      return trimmedText;
    } catch (error) {
...
async function translateSelectedText(text?: string): Promise<void> {
  try {
    ...
    console.log(
      `[Translation] Completed: "${selectedText}" -> "${result.translatedText}"`
    );
  } catch (error) {
...
async function showNamingOptions(): Promise<void> {
  ...
  if (
    ...
    try {
      ...
      const result = await translatorService.translate(selectedText);
      englishText =
        result.sourceLanguage === "en" ? selectedText : result.translatedText;
    } catch (error) {
      console.error("Translation error:", error);
